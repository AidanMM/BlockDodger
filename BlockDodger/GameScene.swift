...
class GameScene: SKScene, UIGestureRecognizerDelegate, SKPhysicsContactDelegate {
    ...
    func respondToSwipeGesture(gesture: UIGestureRecognizer) {
        ...
        if let swipeGesture = gesture as? UISwipeGestureRecognizer {
            var playerAction = SKAction.moveTo(CGPointMake(player.position.x, player.position.y), duration: 0.01)
            ...
            case UISwipeGestureRecognizerDirection.Up:
                if player.position.y + player.size.height < self.size.height {
                    playerAction = SKAction.moveTo(CGPointMake(player.position.x, player.position.y + player.size.height), duration: 0.01)
                }
            default:
    ...
    func reScale(scale: CGFloat) {
        scaleFactor = scale;
        total = Int(self.size.height / scaleFactor / player.size.width)
    ...
    func generateBlockRow() {
        let newScale = CGFloat(Float(arc4random()) / Float(UINT32_MAX)) * 3.0
        reScale( newScale > 1.0 ? newScale : 1.0)
        ...
        for var i = 0; i < total; i++ {
            if i != emptySlot {
                ...
                let actionMoveDone = SKAction.runBlock( {
                    ...
                    //player.setScale(newScale > 1.0 ? newScale : 1.0)
                })
